...
fn main() {
  ...
  let matches = App::new("Etxe")
    ...
    .subcommand(App::new(SUB_CHECK).about("check the syntax of configuration files"))
    .subcommand(App::new(SUB_VALIDATE).about("TODO"))
    .subcommand(App::new(SUB_INSPECT).about("TODO"))
    .get_matches();

  let input: Vec<_> = matches.values_of("input").unwrap().collect();

  match matches.subcommand() {
    (SUB_CHECK, Some(subcommand)) => check_input(&input, subcommand),
    (_, None) => {
