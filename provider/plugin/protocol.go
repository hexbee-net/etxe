...
type ServerProtocol interface {
	...
	// not start listening.
	// This is the point at which all validation should be done and errors
	// returned.
	Init() error

	// Config is extra configuration to be outputted to stdout.
	// This will be automatically base64 encoded to ensure it can be parsed
	// properly.
	// This can be an empty string if additional configuration is not needed.
	Config() string

	// Serve is called to serve connections on the given listener.
	// This should continue until the listener is closed.
	Serve(net.Listener)
}
